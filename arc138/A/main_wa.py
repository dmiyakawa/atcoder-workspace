...
def solve(N: int, K: int, A: "List[int]"):
    ...
    left_most_index = {}
    for i in range(K, N):
        if A[i] not in left_most_index:
            left_most_index[A[i]] = i
        ss.add(A[i])

    min_movement = None

    for k in range(K - 1, -1, -1):
        val = ss.gt(A[k])  # ここがまちがい
        if val is not None:
            new_val = (K - 1 - k) + (left_most_index[val] - K) + 1
            if min_movement is None:
                min_movement = new_val
            else:
                min_movement = min(min_movement, new_val)
