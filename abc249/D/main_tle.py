...
def main():
    ...
    for i, ai in enumerate(A_s):
        ai_sqrt = math.sqrt(abs(ai))
        for j, aj in enumerate(A_s):
            if abs(aj) > ai_sqrt:
                break
            if ai % aj == 0:
                ak = ai // aj
                if ak == aj:
                    # count += A_ms.count(ai) * A_ms.count(ak) ** 2
                    count += A_count[ai] * A_count[ak] ** 2
                else:
                    count += 2 * A_count[ai] * A_count[aj] * A_count[ak]
    print(count)
