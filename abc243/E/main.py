...
def main():
    ...
    for n in range(1, N + 1):
        for node in do_dijkstra(N, links, start_id=n):
            prev = node.prev
            if prev:
                a = min(node.node_id, prev.node_id)
                b = max(node.node_id, prev.node_id)
                links_2.discard((a, b))
    print(len(links_2), end="")
