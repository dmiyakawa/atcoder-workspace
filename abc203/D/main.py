...
def solve(N: int, K: int, A: "List[List[int]]"):
    ...
    mid = K ** 2 - int((K ** 2)/2)
    lst.sort()
    cur_median = lst[mid - 1]
    s1 = SortedMultiset(lst[:mid])
    for i in range(N - K + 1):
        if i > 0:
            i_top = i - 1
            i_bottom = i + K - 1
            for l in range(K):
                s1.discard(A[i_top][l])
                if cur_median >= A[i_bottom][l]:
                    s1.add(A[i_bottom][l])
            while len(s1) > mid:
                s1.remove(s1[-1])
            if len(s1) == mid:
                cur_median = s1[-1]
        s2 = SortedMultiset(s1)
        for j in range(1, N - K + 1):
            j_left = j - 1
            j_right = j + K - 1
            for v in range(K):
                s2.discard(A[v][j_left])
                if cur_median >= A[v][j_right]:
                    s2.add(A[v][j_right])
            while len(s2) > mid:
                s2.remove(s2[-1])
            if len(s2) == mid:
