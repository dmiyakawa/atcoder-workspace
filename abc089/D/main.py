...
def solve(H: int, W: int, D: int, A: "List[List[int]]", Q: int, L: "List[int]", R: "List[int]"):
    ...
    farthest_rs = set()
    for l in L:
        farthest_rs.add(((H * W - l) // D) * D + l)

    distances = {}
    for r in farthest_rs:
        l = r
        distances[(r, r)] = 0
        while l > D:
            l = l - D
            distances[(l, r)] = distances[(l + D, r)] + _d(l, l + D)

    for l, r, in zip(L, R):
        farthest = ((H * W - l) // D) * D + l
        print(distances[(l, farthest)] - distances[(r, farthest)])
